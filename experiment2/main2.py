...
def retrieve_similar_texts(query, top_k=1): ###-----------------------------------------------změnit 
    try:
...
def save_results_to_file(query, response, min_score, max_score, token_usage, prompt_tokens, completion_tokens, filename="outputs2.txt"):
    with open(filename, "a", encoding="utf-8") as file:
        ...
        file.write(f"Verze experimentu: 2c\n")  ###------------------------------------------------------změnit
...
st.write("Experimentální Verze 2c") ##############------------------------------------------ změnit
...
if query:
    with st.spinner("Vyhledávání relevantních textů..."):
        ...
        retrieved_texts = retrieve_similar_texts(query, top_k=1) ##############------------------------------------------ změnit
