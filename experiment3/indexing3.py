...
# Import modulu pro načítání dat ze souboru
import json
import re
import requests
...
with open('/content/text_pairs3.txt', 'r', encoding='utf-8') as file:
    texts_and_metadata = json.load(file)

# Načtení textů a metadat ze souboru
with open('/content/text_pairs_resp3.txt', 'r', encoding='utf-8') as file:
    texts_and_metadata_resp = json.load(file)

# Smyčka pro generování embeddingů a jejich ukládání do Pinecone pro první soubor
...
# Smyčka pro generování embeddingů a jejich ukládání do Pinecone pro druhý soubor
for j, item in enumerate(texts_and_metadata_resp, start=1001):
    # Text a metadata z aktuální položky
    text = item["question"]
    metadata = item["metadata"]
    ...
    # Vytvoření jedinečného vector_id s ID od 1001 dál
    vector_id = f"text_response_{j}"
